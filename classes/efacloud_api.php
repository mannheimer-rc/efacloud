...
class Efacloud_api
{
    ...
    public static $result_codes = [300 => "Transaction completed.",400 => "XHTTPrequest Error.",
            ...
            504 => "Transaction container decoding failed.",505 => "Server response empty",
            506 => "Internet connection aborted",507 => "Could not decode server response"
    ];
    ...
    /**
     * Encode a plain text container. This converts the String first to UTF-8, encodes the result in
     * base64 and replaces the characters "=/+" by "_-*" respectively.
     *
    ...
    /**
     * Decode a plain text container. This replaces the characters "_-*" by "=/+" respectively. then
     * decodes the base64 sequence and finally decodes the resulting UTF-8 String to PHP native.
     *
    ...
    private function create_container ()
    {
        ...
        $txc_plain = $this->txc_header["version"] . ";" . $this->txc_header["cID"] . ";" .
        ...
        $txc_plain = substr($txc_plain, 0, 
                strlen($txc_plain) - strlen(self::$transaction_separator));
        return $txc_plain;
    }
     ...
     *            the transaction messages as non-associative array. Use
     *            "Tx_handler->create_request()" to create those
     ...
     * @return the result as [(int) result_code, (String) result_message]. If the container has not
     *         been sent, this will be an error code and message.
     */
    public function get_result (int $tx_id)
    {
        if ($this->txc_open)
            return [502,
                    "The transaction has not been send, or is still waiting for a response."
            ];
        if (! isset($this->txc_messages[$tx_id]))
            return [502,"The requested transaction is not in the container."
            ];
        return [$this->txc_messages[$tx_id]["result_code"],
                $this->txc_messages[$tx_id]["result_message"]
        ];
    }
}
