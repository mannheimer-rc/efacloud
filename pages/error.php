...
if (($last_error !== false) && (count(explode(";", $last_error)) >= 3)) {
    ...
    $headline = "Unbestimmter Fehler";
    $text = "Der verursachende Fehler konnte nicht mehr ermittelt werden.";
}
...
if ($too_many_sessions) {
    ...
    echo "<html><body><h4>Überlast der Website</h4><p>" . $headline . "</p><p>" . $text .
             "</p></body></html>";
...
if ($suppress_throttling)
    ...
    $toolbox->load_throttle("errors", 
            $toolbox->config->settings_tfyh["init"]["max_errors_per_hour"], "error in " . $source_file);
    $toolbox->logger->log_init_login_error("error");
}
